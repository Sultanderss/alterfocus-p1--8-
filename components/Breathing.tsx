...
interface BreathingProps {
  onComplete: (result?: 'yes' | 'no' | 'cancelled') => void;
}
...
const Breathing: React.FC<BreathingProps> = ({ onComplete }) => {
  ...
  return (
    <motion.div
      className={`fixed inset-0 z-[60] flex flex-col overflow-hidden bg-gradient-to-br ${MODES[selectedMode].bgGradient} text-white transition-colors duration-1000`}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
    >
      ...
      <AnimatePresence mode="wait">
        {step === 'intro' && (
          ...
          >
            ...
            <button onClick={() => onComplete('cancelled')} className="mt-12 text-white/40 text-sm hover:text-white transition-colors">
              Cancelar
            </button>
          </motion.div>
        )}
